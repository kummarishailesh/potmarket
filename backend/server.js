...
app.get('/api/admin/users', requireAdmin, (req, res) => { const query = sanitizeText(req.query.search).toLowerCase(); const users = db.getUsers().filter(user => !query || `${user.name} ${user.email} ${user.phone || ''}`.toLowerCase().includes(query)).map(user => { const orders = db.getOrdersByUser(user.id); const latestOrder = [...orders].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0]; return { ...publicUser(user), phone: user.phone || latestOrder?.shippingAddress?.phone || '', shippingAddress: user.shippingAddress || latestOrder?.shippingAddress || null, orderCount: orders.length, totalSpent: orders.filter(order => orderPaymentStatus(order) === 'Paid').reduce((sum, order) => sum + Number(order.total || 0), 0) }; }); return res.json({ success: true, users }); });
