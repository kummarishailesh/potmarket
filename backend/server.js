...
app.get('/api/admin/users', requireAdmin, (req, res) => {
    ...
    const users = [...records.values()].filter(({ user }) => !query || `${user.name} ${user.email} ${user.phone || ''}`.toLowerCase().includes(query)).map(({ user, orders }) => {
        ...
        return { ...publicUser(user), phone: user.phone || latestOrder?.shippingAddress?.phone || '', shippingAddress: user.shippingAddress || latestOrder?.shippingAddress || null, orderCount: orders.length, totalSpent: orders.filter(order => orderPaymentStatus(order) === 'Paid').reduce((sum, order) => sum + Number(order.total || 0), 0) };
...
app.get('/api/admin/notifications', requireAdmin, (req, res) => res.json({ success: true, notifications: [] }));
